#include "caffe/layers/dtransform_layer.hpp"

namespace caffe {

template <typename Dtype>
__global__ void dtransform_gpu(const int nthreads, const Dtype* from_data, Dtype* to_data, 
...
template <typename Dtype>
void DTransformLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, 
		const vector<Blob<Dtype>*>& top) {
	const int nthreads = bottom[0]->count();

	dtransform_gpu<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
...
template <typename Dtype>
void DTransformLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(DTransformLayer);
